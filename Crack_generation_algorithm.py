...
def perlin(x, y, perm):
    # Computes ONE Perlin noise value at coordinate (x, y).
    # 'perm' is a shuffled permutation table used as a repeatable source
    # of "randomness" so neighboring points get correlated (smooth) noise.
...
def draw_crack(img, x, y, angle, length, depth=CRACK_DEPTH):
    ...
    for i in range(int(length)):
        ...
        if 0 <= ix < w and 0 <= iy < h:
            # Width tapering: crack starts thick and tapers thinner toward
            # its tail
    ...
    if len(points) > 2:
        ...
        # so there are no visible gaps along the path.
    ...
    # Paste the small noise patch onto a full-image-sized canvas so it
    ...
    # Nonlinear (sigmoid) threshold: pushes mid-range alpha values toward
