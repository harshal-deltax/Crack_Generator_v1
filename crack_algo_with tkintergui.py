...
def draw_crack(img, x, y, angle, length, depth=CRACK_DEPTH):
    ...
    for i in range(int(length)):
        ...
        step = 1.0
        cur_x += np.cos(np.radians(cur_angle)) * step
        cur_y += np.sin(np.radians(cur_angle)) * step
        ix, iy = int(cur_x), int(cur_y)
        if 0 <= ix < w and 0 <= iy < h:

            # craack width variation
            px_width = max(1, int(depth * (1 -  0.35 * i / length)))
                        
#width crack with variation         
           
            #px_width = max(1, int(depth + np.random.uniform(-2,2)))
            base_width = depth
            random_offset = np.random.uniform(-1, 1)
            #px_width = max(CRACK_DEPTH, int(base_width + random_offset))
            #px_width = max(1, int(base_width + random_offset))
            #px_width = CRACK_DEPTH
    ...
    noise_smooth = cv2.GaussianBlur(noise_canvas, (3,3), 1)

    #noise_mod = 0.5 + 0.5 * noise_smooth
    noise_mod = 0.5 + 0.5 * noise_smooth
    ...
    # Smooth crack edge
    alpha = cv2.GaussianBlur(alpha, (3,3), 0.3)
...
#dark variation
# This controls how the darkness changes along the crack.
    variation = cv2.GaussianBlur(noise_canvas, (3,3), 0.9)
    #variation  = 0.9 + 0.1 * variation
    variation  =  0.9 + 0.1* variation
   
   
   # crack strenght darker/lighter
    strength    = 3   # increase for deeper/darker crack
  ...
  # 5. Write crack colour directly, strictly inside crack_mask (no alpha blending)
    crack_color = np.clip(crack_color, 0, 255).astype(np.uint8)

    mask3 = np.stack([crack_mask, crack_mask, crack_mask], axis=-1) > 0
    img[mask3] = crack_color[mask3]
