...
def draw_crack(
    ...
    reflect_rim_inner=0.02,    # how close to the dark core the glint starts (0-1, LOCAL width-relative, lower = closer/overlapping)
    reflect_rim_outer=0.18,    # how far out the glint extends (0-1, LOCAL width-relative). Lower = THINNER band. This now scales with the crack's *local* thickness instead of its global max thickness.
    ...
    reflect_color= (200,214,207),        # NEW: e.g. (180, 220, 255) for icy blue, (255, 200, 120) for gold. None = old lighten behavior
):
    ...
    # 3b. Local width map: for every pixel, the half-width of the crack at
    # that *point along the crack* (not the crack's single widest spot).
    # This lets the rim/reflection band be measured as a fraction of the
    # crack's local thickness, so it comes out thin where the crack is thin
    # and only proportionally wider where the crack is actually wide.
    ...
    # Measured against dist_n_rim (LOCAL width), so the band's absolute
    # pixel thickness now tracks the crack's thickness at that exact point,
    # instead of being sized off the crack's single widest spot.
    #
    # The dark core (alpha) is darkened using the GLOBAL dist_n scale (so the
    # crack still tapers realistically along its length), while the rim is
    # measured on the LOCAL scale (so its thickness tracks local crack width).
    # Those two scales only coincide at the crack's single widest point, so
    # naively using a small fixed reflect_rim_outer reopens a gap between the
    # dark core and the highlight everywhere else (worse for thin sections).
    # Convert the global "where darkening starts" threshold into local units,
    # per pixel, and make sure the rim's inner reach always meets it, with a
    # small overlap margin — this removes the gap without puffing the band
    # out to a fixed large width everywhere like before.
    GAP_OVERLAP_MARGIN = 0.05
    local_dark_edge = np.clip(
        (SIGMOID_CENTER * dist.max()) / (width_map + 1e-6), 0.0, 1.0
    )
    rim_outer_eff = np.maximum(reflect_rim_outer, local_dark_edge + GAP_OVERLAP_MARGIN)

    rim_mask = np.where(
        (crack_mask > 0)
        & (dist_n_rim >= reflect_rim_inner)
        & (dist_n_rim <= rim_outer_eff),
        1.0, 0.0
    ).astype(np.float32)
